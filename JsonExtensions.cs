...
namespace JsonFileWrapper {
	public static class JsonExtensions {
		...
		public static readonly char NestedPropertyDelimiter = '.';
		...
		public static void NestedSet( this JsonObject jsonObject, string propertyPath, JsonNode? value ) {
			...
			for ( int index = 0; index < propertyNames.Length; index++ ) {
				...
				if ( ( index + 1 ) == propertyNames.Length ) {
					...
					JsonNode? node = nextObject[ propertyName ];

					if ( node == null ) throw new JsonPropertyNullException( $"Nested property '{propertyName}' in '{propertyPath}' is null, it cannot be used as the next object" );

					nextObject = node.AsObject();
